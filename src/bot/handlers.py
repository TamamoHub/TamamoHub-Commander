...
def setup_handlers(bot):
    ...
    def start(message):
        ...
        if not admin_id:
            ...
            log_to_console(f"New admin set: {message.from_user.username} (ID: {message.from_user.id})")
            bot.reply_to(
                message,
            ...
            log_to_console(f"Admin {message.from_user.username} started bot")
            bot.reply_to(
                message,
            ...
            log_to_console(f"Unauthorized access attempt from user {message.from_user.username} (ID: {message.from_user.id})")
            bot.reply_to(
                message,
    ...
    def handle_inline_buttons(call):
        try:
            ...
            if not admin_id or call.from_user.id != admin_id:
                log_to_console(f"Unauthorized callback attempt from user {call.from_user.username} (ID: {call.from_user.id})")
                bot.answer_callback_query(call.id, "Доступ заборонено")
                return

            # Додаємо обробку кнопок керування ботом
            if call.data == "shutdown_program":
                log_to_console(f"User {call.from_user.username} initiated program shutdown")
                ...
                log_to_console(f"User {call.from_user.username} requested log file")
                ...
                if action == "drive":
                    drive = params[0]
                    log_to_console(f"User {call.from_user.username} accessed drive {drive}")
                    path = f"{drive}:\\"
                    bot.edit_message_text(
                        f"📂 {path}",
                        call.message.chat.id,
                    ...
                    path = ":".join(params)
                    log_to_console(f"User {call.from_user.username} opened folder: {path}")
                    bot.edit_message_text(
                        f"📂 {path}",
                        call.message.chat.id,
                        call.message.message_id,
                    ...
                    log_to_console(f"User {call.from_user.username} accessed file: {file_path}")
                    bot.edit_message_text(
                ...
                if action == "main":
                    ...
                    pid = int(params[0])
                    log_to_console(f"User {call.from_user.username} attempting to kill process {pid}")
                    ...
                    pid = int(params[0])
                    log_to_console(f"User {call.from_user.username} attempting to restart process {pid}")
                ...
                log_to_console(f"User {call.from_user.username} executed Windows command: {command}")
                ...
                log_to_console(f"User {call.from_user.username} executed mouse command: {command} {' '.join(args)}")
                ...
                _, command, *args = call.data.split(":")
                log_to_console(f"User {call.from_user.username} executed keyboard command: {command}")
                ...
                _, command = call.data.split(":")
                log_to_console(f"User {call.from_user.username} accessed advanced feature: {command}")
            ...
            log_error(f"Error handling callback from user {call.from_user.username}", e)
    ...
    def handle_keyboard_input(message):
        ...
        log_to_console(f"User {message.from_user.username} sending keyboard input")
    ...
    def handle_pc_text(message):
        if not is_admin(message):
            return
        log_to_console(f"User {message.from_user.username} sending text to PC: {message.text[:50]}...")
        text = message.text.strip()
        if text:
    ...
    def handle_tts(message):
        if not is_admin(message):
            return
        log_to_console(f"User {message.from_user.username} requested TTS: {message.text[:50]}...")
        text = message.text.strip()
        if text:
    ...
    def handle_math(message):
        if not is_admin(message):
            return
        log_to_console(f"User {message.from_user.username} requested math calculation: {message.text}")
    ...
    def handle_url(message):
        if not is_admin(message):
            return
        log_to_console(f"User {message.from_user.username} requested to open URL: {message.text}")
    ...
    def handle_voice(message):
        ...
        try:
            log_to_console(f"User {message.from_user.username} sent voice message for recognition")
            ...
            log_error(f"Error processing voice message from user {message.from_user.username}", e)
